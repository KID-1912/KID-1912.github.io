...
标识值列表：[Key Values - Web API | MDN](https://developer.mozilla.org/zh-CN/docs/Web/API/UI_Events/Keyboard_event_key_values)
...
```js
const resizeObserver = new ResizeObserver(entrires => {
  for(const entry of entries) {
    scrollHeight = entry.contentRect.height; // contentRect即盒子尺寸信息(旧版本)
  }
})
```
...
```js
resizeObserver.observe($container); // 观察
resizeObserver.disconnect(); // 销毁
```

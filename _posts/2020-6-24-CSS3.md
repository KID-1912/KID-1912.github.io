...
- steps(start, n) 立即开始, 忽略起始帧，起于第二帧向后到最后一帧结束

- steps(end, n) 立即结束, 忽略结束帧，起于第一帧向后到倒数第二帧结束

- step-start steps(start, 1)

- step-end steps(end, 1)
...
3. 检查动画，是否需要补帧，是否需要调整为steps(start,n)，是否需要animation-fill-mode
